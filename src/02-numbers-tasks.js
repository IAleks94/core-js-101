...
function getAverage(value1, value2) {
  ...
  const sum = Math.round(value1) + Math.round(value2);
  return sum ? (sum / 2) : sum;
}
 ...
 *   1678, 3  => 2000
 */
function roundToPowerOfTen(num, exp) {
...
function isPrime(n) {
  ...
  const answer = primeArr.map((item) => n % item);
  if (primeArr.includes(n)) {
    return true;
  }
  return !answer.includes(0);
}
